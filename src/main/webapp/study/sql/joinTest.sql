...
insert into student values (default, '생성X', 107, default); -- 107번 필드값이 없으므로 오류발생
...
-- LEFT JOIN(왼쪽 기준) : 왼쪽 자료를 모두 가져오고 오른쪽 자료는 만족하는것만 가져온다 (없을경우:NULL)
select * from hakkwa h LEFT JOIN student s on h.code = s.hakkwa_code;

-- RIGHT JOIN(오른쪽 기준) : 오른쪽 자료를 모두 가져오고 왼쪽 자료는 만족하는것만 가져온다 (없을경우:NULL)
...
/* 합집합 : Full outer Join(MySQL에 없음), MySQl에서는 UNION명령을 사용한다 */
