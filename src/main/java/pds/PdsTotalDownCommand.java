...
public class PdsTotalDownCommand implements PdsInterface {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// DB에 저장된 파일의 정보(원본 파일 | 시스템에 저장된 파일)을 가져와서 각각의 파일로 분리작업하기
		...
		// 서버에서 압축작업이 완료되면 압축파일을 클라이언트로 전송하고, 서버에 존재하는 압축파일을 삭제하기
		...
		sos.close();
		fis.close();
		// 클라이언트에 압출파일을 전송 완료하기
		
		// 서버에 존재하는 zip파일을 삭제하기
