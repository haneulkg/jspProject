...
public class PassOk1 extends HttpServlet{
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(idx == 1) {
			// 숫자만을 암호화 하는 경우
			// 암호화를 위한 암호화키(0x1234ABCD) -- Java는 앞에 0x를 붙임
			int key = 0x1234ABCD;
			
			int encPwd, decPwd;
			// 암호화 하기 (EOR : 배타적OR)암호화 => 이진수 EOR : ^
			...
			out.println("<script>");
			out.println("alert('원본 비밀번호 : "+pwd+"\\n암호화된 비밀번호 : "+encPwd+"')");
			out.println("location.href='"+request.getContextPath()+"/study/password/passForm.jsp';");
			out.println("</script>");
		}
		else if(idx == 2) {
			System.out.println();
			// 숫자와 영문자(대/소)의 혼합(아스키코드 2자리로 변환후 암호화 처리하도록 한다 : 영문 소문자로 입력시에는 대문자로 변경하여 처리하기)
			long intPwd;
			String strPwd = "";
			System.out.println("1. 원본 비밀번호 (예:ABCD) : " + pwd);
			
			for(int i=0; i<pwd.length(); i++) {
				intPwd = pwd.charAt(i); // charAt() : 한글자씩 꺼냄
				strPwd += intPwd;
			}
			...
			// 복호화(복호화 되는 비밀번호는 숫자이기에 2개씩 분리시켜서 처리한다)
